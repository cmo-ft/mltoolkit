...
class AttentionBlock(nn.Module):
    ...
    def forward(self, node_attr, edge_index, batch):
        ...
        node_out = self.norm1(self.mhsa(x=node_attr, edge_index=edge_index))
...
class Transformer(torch.nn.Module):
    ...
    def forward(self, batch):
        pos = batch.pos if batch.pos is not None else batch.node_features
        ...
        for idx, layer in enumerate(self.conv_process):
            node_attr = layer(node_attr, edge_index, batch.batch)
